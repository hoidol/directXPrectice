...
void D3DClass::BeginScene(float red, float green, float blue, float alpha)
{
	...
	this->m_deviceContext->ClearRenderTargetView(m_renderTargetView, color);
	this->m_deviceContext->ClearDepthStencilView(m_depthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);

	this->m_deviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY::D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	this->m_deviceContext->RSSetState(m_rasterState);
	this->m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 0);
